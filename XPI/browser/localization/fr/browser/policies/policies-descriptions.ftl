...
policy-AppAutoUpdate = Activer ou désactiver la mise à jour automatique des applications.
...
policy-DisabledCiphers = Désactiver les ciphers.
...
policy-DisablePasswordReveal = Ne laissez pas les mots de passe être révélés dans les connexions enregistrées.
...
policy-LegacyProfiles = Désactiver la fonctionnalité imposant un profil distinct pour chaque installation
...
policy-OfferToSaveLoginsDefault = Définissez la valeur par défaut pour autoriser { -brand-short-name } à proposer de mémoriser les identifiants et mots de passe enregistrés. Les valeurs vraies et fausses sont acceptées.
...
policy-PasswordManagerEnabled = Activez l'enregistrement des mots de passe dans le gestionnaire de mots de passe.

policy-Permissions2 = Configure les permissions pour la caméra, le microphone, la localisation et les notifications.
